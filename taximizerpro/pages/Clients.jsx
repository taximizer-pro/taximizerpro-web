    ...
    <div className="min-h-screen bg-[#080F1E] text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div className="space-y-2">
          {loading ? (
            <div className="bg-[#0D1628] border border-white/5 rounded-2xl py-16 text-center text-slate-500 text-sm">Loading...</div>
          ) : clients.length === 0 ? (
            <div className="bg-[#0D1628] border border-white/5 rounded-2xl py-16 text-center">
            ...
            return (
              <Link key={c.id} to={createPageUrl("ClientDetail")+"?id="+c.id}
                className="flex items-center gap-4 bg-[#0D1628] border border-white/5 hover:border-amber-400/20 rounded-2xl px-5 py-4 transition-all group">
                ...
                <div className="hidden sm:flex items-center gap-2">
                  {yrs.slice(0,3).map(y => (
                    <span key={y} className="text-xs font-medium px-2 py-1 rounded-lg bg-white/5 text-slate-400">{y}</span>
                  ))}
                </div>
