...
def fill_form(tmpl_bytes, yr, c):
    ...
    tok  = {"FM":fm,"LN":c.get("last_name",""),"SSN":ssn,"ADDR":addr,
             "CITY":c.get("city",""),"ST":c.get("state",""),"ZIP":c.get("zip",""),
             "RT":c.get("bank_routing",""),"AC":c.get("bank_account",""),"OCC":"HELPER"}
    ...
    for w in doc[0].widgets():
        ...
        if w.field_type_string == "Text" and sn in P1.get(yr,{}) and tok.get(P1[yr][sn]):
            w.field_value = tok[P1[yr][sn]]; w.update()
    for w in doc[1].widgets():
        sn = w.field_name.split(".")[-1]
        if w.field_type_string == "Text" and sn in P2.get(yr,{}) and tok.get(P2[yr][sn]):
            w.field_value = tok[P2[yr][sn]]; w.update()
    dx, dy = DATE_XY[yr]
    doc[1].insert_text((dx, dy), today, fontname="helv", fontsize=7, color=(0,0,0))

    # Signature from base64 data
...
def send_notification(to, first_name, links):
    rows = "".join(f'<tr><td style="padding:10px 16px;border-bottom:1px solid #e2e8f0"><a href="{lnk}" style="color:#d97706;font-weight:bold">📄 {yr} Form 1040</a></td><td style="padding:10px 16px;border-bottom:1px solid #e2e8f0;color:#64748b;font-size:13px">View →</td></tr>' for yr,lnk in sorted(links.items()))
    html = (f'<div style="font-family:Arial,sans-serif;max-width:580px;margin:0 auto">'
            f'<div style="background:linear-gradient(135deg,#f59e0b,#d97706);padding:20px 24px"><span style="font-size:20px;font-weight:900;color:#fff">TaximizerPro</span></div>'
            f'<div style="padding:24px;background:#fff;border:1px solid #e2e8f0"><h2 style="color:#1e293b">Your Tax Forms Are Ready ✅</h2>'
            f'<p style="color:#475569">Hi <strong>{first_name}</strong>, your IRS 1040 forms are ready.</p>'
            f'<table style="width:100%;border-collapse:collapse;border:1px solid #e2e8f0"><thead><tr style="background:#f8fafc">'
            ...
            f'<p style="font-size:12px;color:#94a3b8;margin-top:20px;padding-top:12px;border-top:1px solid #e2e8f0">Nobody controls the IRS — not even Italy, and that says a lot...<br><strong>TaximizerPro</strong></p>'
            f'</div></div>')
...
def api_stats():
    ...
    try:
        ...
        pending = sum(1 for c in records if c.get("filing_status") == "pending")
